...
// shared library is not recommended, because of function call overhead and
...
#define IM_UNICODE_CODEPOINT_MAX \
  0xFFFF  // Last Unicode code point supported by this build.
#define IM_UNICODE_CODEPOINT_INVALID \
  0xFFFD  // Standard invalid Unicode code point.
...
// programming IDE "Go to definition" facility on the names in the central
// column below to find the actual flags/enum lists.
#ifndef ImTextureID
typedef void*
...
IMGUI_API void SetWindowFontScale(
    float scale);  // set font scale. Adjust IO.FontGlobalScale if you want to
                   // scale all windows. This is an old API! For correct
                   // scaling, prefer to reload font + rebuild ImFontAtlas +
...
// and output at the right of the preceeding widget.
IMGUI_API void
...
// (*) You can use IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup) to
...
// - By end of the 2019 we will expose a new 'Table' api which will replace
// columns.
...
// Obsolete names (will be removed)
  ...
  ImGuiTreeNodeFlags_AllowOverlapMode =
      ImGuiTreeNodeFlags_AllowItemOverlap  // [renamed in 1.53]
#endif
};
...
// Pre 1.71 back-ends will typically ignore the VtxOffset/IdxOffset fields. When
// 'io.BackendFlags & ImGuiBackendFlags_RendererHasVtxOffset' is enabled, those
// fields allow us to render meshes larger than 64K vertices while keeping
// 16-bit indices.
struct ImDrawCmd {
  unsigned int
  ...
  unsigned int VtxOffset;  // Start offset in vertex buffer. Pre-1.71 or without
                           ...
                           // With ImGuiBackendFlags_RendererHasVtxOffset: may
                           // be >0 to support meshes larger than 64K vertices
                           // with 16-bit indices.
  unsigned int IdxOffset;  // Start offset in index buffer. Always equal to sum
                           // of ElemCount drawn so far.
  ImDrawCallback
...
// the renderer back-end) (to use 32-bit indices: override with '#define
...
struct ImDrawList {
  ...
  unsigned int _VtxCurrentOffset;  // [Internal] Always 0 unless 'Flags &
                                   // ImDrawListFlags_AllowVtxOffset'.
  unsigned int
...
struct ImFontAtlasCustomRect {
  unsigned int
      ID;  // Input    // User ID. Use < 0x110000 to map into a font glyph, >=
           // 0x110000 for other/internal/custom texture data.
  unsigned short Width, Height;  // Input    // Desired rectangle dimension
  unsigned short X, Y;           // Output   // Packed position in Atlas
  float GlyphAdvanceX;  // Input    // For custom font glyphs only (ID <
                        // 0x110000): glyph xadvance
  ImVec2 GlyphOffset;   // Input    // For custom font glyphs only (ID <
                        // 0x110000): glyph display offset
  ImFont* Font;  // Input    // For custom font glyphs only (ID < 0x110000):
                 // target font
...
struct ImFontAtlas {
  ...
  // colorful icons and use them as regular glyphs. Read docs/FONTS.txt for more
  // details about using colorful icons.
  IMGUI_API int AddCustomRectRegular(
      unsigned int id, int width,
      int height);  // Id needs to be >= 0x110000. Id >= 0x80000000 are reserved
                    // for ImGui and ImDrawList
  IMGUI_API int AddCustomRectFontGlyph(
      ImFont* font, ImWchar id, int width, int height, float advance_x,
      const ImVec2& offset =
          ImVec2(0, 0));  // Id needs to be < 0x110000 to register a rectangle
...
struct ImFont {
  ...
  ImWchar
      FallbackChar;  // 2     // in  // = '?'      // Replacement character if a
                     // glyph isn't found. Only set via SetFallbackChar()
