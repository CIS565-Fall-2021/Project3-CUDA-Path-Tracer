...
// debugging/work, otherwise would slow down the code too much.
...
enum ImGuiButtonFlags_ {
  ...
  ImGuiButtonFlags_NoKeyModifiers =
      1 << 10,  // disable mouse interaction if a key modifier is held
...
enum ImGuiSelectableFlagsPrivate_ {
  ...
  ImGuiSelectableFlags_DrawFillAvailWidth =
      1 << 23,  // FIXME: We may be able to remove this (added in 6251d379 for
                // menus)
...
struct ImGuiContext {
  ...
  ImU32 ActiveIdUsingNavDirMask;    // Active widget will want to read those
                                    // directional navigation requests (e.g. can
                                    // activate a button and move away from it)
  ImU32 ActiveIdUsingNavInputMask;  // Active widget will want to read those nav
                                    // inputs.
  ImU64 ActiveIdUsingKeyInputMask;  // Active widget will want to read those key
...
struct IMGUI_API ImGuiWindowTempData {
  ...
  ImU32 TreeMayJumpToParentOnPopMask;  // Store a copy of !g.NavIdIsAlive for
                                       // TreeDepth 0..31.. Could be turned into
...
struct IMGUI_API ImGuiWindow {
  ...
  // The best way to understand what those rectangles are is to use the 'Metrics
  // -> Tools -> Show windows rectangles' viewer. The main 'OuterRect', omitted
...
struct ImGuiTabItem {
  ...
  float ContentWidth;  // Width of actual contents, stored during BeginTabItem()
                       // call
...
IMGUI_API bool IsMouseDragPastThreshold(int button,
...
IMGUI_API float GetColumnOffsetFromNorm(const ImGuiColumns* columns,
                                        float offset_norm);
IMGUI_API float GetColumnNormFromOffset(const ImGuiColumns* columns,
                                        float offset);
...
inline void DebugDrawItemRect(ImU32 col = IM_COL32(255, 0, 0, 255)) {
  ...
  GetForegroundDrawList(window)->AddRect(window->DC.LastItemRect.Min,
                                         window->DC.LastItemRect.Max, col);
}
