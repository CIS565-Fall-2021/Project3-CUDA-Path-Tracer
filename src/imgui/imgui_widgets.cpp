  ...
  if (g.DragDropActive && (flags & ImGuiButtonFlags_PressedOnDragDropHold) &&
    ...
    if (IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem)) {
      ...
      if (CalcTypematicRepeatAmount(g.HoveredIdTimer + 0.0001f - g.IO.DeltaTime,
                                    g.HoveredIdTimer + 0.0001f, 0.70f, 0.00f)) {
        pressed = true;
        FocusWindow(window);
      }
    }
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis,
  ...
  if (held && allow_interaction && grab_h_norm < 1.0f) {
    ...
    // after the calculation of ContentSize and before setting up our starting
    // position
...
void ImGui::Scrollbar(ImGuiAxis axis) {
  ...
  if (axis == ImGuiAxis_X) {
    ...
    bb.Min = ImVec2(ImMax(outer_rect.Min.x,
  ...
  ScrollbarEx(bb, id, axis, &window->Scroll[axis],
              inner_rect.Max[axis] - inner_rect.Min[axis],
...
bool ImGui::Checkbox(const char* label, bool* v) {
  ...
  if (window->DC.ItemFlags & ImGuiItemFlags_MixedValue) {
    ...
    RenderCheckMark(check_bb.Min + ImVec2(pad, pad), check_col,
                    square_sz - pad * 2.0f);
  }
...
bool ImGui::ColorEdit4(const char* label, float col[4],
  ...
  if ((flags & (ImGuiColorEditFlags_DisplayRGB |
    ...
    for (int n = 0; n < components; n++) {
      ...
      const bool disable_hue_edit =
          (n == 0 && (flags & ImGuiColorEditFlags_DisplayHSV) && i[1] == 0);
      if (flags & ImGuiColorEditFlags_Float) {
        value_changed |= DragFloat(ids[n], &f[n], 1.0f / 255.0f,
                                   disable_hue_edit ? +FLT_MAX : 0.0f,
                                   disable_hue_edit ? -FLT_MAX
                                   : hdr            ? 0.0f
                                                    : 1.0f,
        ...
        value_changed |=
            DragInt(ids[n], &i[n], 1.0f, disable_hue_edit ? INT_MAX : 0,
                    disable_hue_edit ? INT_MIN
                    : hdr            ? 0
                                     : 255,
  ...
  if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel)) {
    window->DC.CursorPos = ImVec2(pos.x + w_full + style.ItemInnerSpacing.x,
...
bool ImGui::ColorPicker4(const char* label, float col[4],
  ...
  if (value_changed) {
    if (flags & ImGuiColorEditFlags_InputRGB) {
      ...
      ColorConvertRGBtoHSV(R, G, B, H, S, V);
      if (S == 0 &&
          memcmp(g.ColorEditLastColor, col, sizeof(float) * 3) ==
  ...
  if (alpha_bar) {
    ...
    RenderColorRectWithAlphaCheckerboard(bar1_bb.Min, bar1_bb.Max, 0,
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
  ...
  const float arrow_hit_x2  = (text_pos.x - text_offset_x) +
                             (g.FontSize + padding.x * 2.0f) + hit_padding_x;
...
void ImGui::TreePop() {
  ...
  if (g.NavMoveDir == ImGuiDir_Left && g.NavWindow == window &&
    ...
    if (g.NavIdIsAlive &&
        (window->DC.TreeMayJumpToParentOnPopMask & tree_depth_mask)) {
