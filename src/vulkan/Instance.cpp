...
Instance::Instance(const char* applicationName, unsigned int additionalExtensionCount, const char** additionalExtensions) {
    ...
    auto instCreateRes = vkCreateInstance(&createInfo, nullptr, &instance);
    // Create instance
    if (instCreateRes != VK_SUCCESS) {
        std::cout << "failure code " << instCreateRes << ' ' << VK_ERROR_LAYER_NOT_PRESENT << '\n';
      ...
      vkGetPhysicalDeviceFeatures(device, &deviceFeatures);
        ...
        if (queueSupport &&
            ...
            (!requiredQueues[QueueFlags::Present] || (!surfaceFormats.empty() && ! presentModes.empty())) &&
                    (deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU || !REQUIRE_NVIDIA)
        ) {
            std::cout << "DEVICE PROP " << deviceProperties.deviceName << '\n';
