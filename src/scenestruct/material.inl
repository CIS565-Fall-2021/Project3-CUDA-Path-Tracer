...
GLM_FUNC_QUALIFIER float Material::fresnel(const glm::vec3& in, const glm::vec3& normal, float ior)
{
...
GLM_FUNC_QUALIFIER MonteCarloPair Material::Phong_sampleScatter_Uniform(const glm::vec3& in, const glm::vec3& normal, const glm::vec2& uv, thrust::default_random_engine& rng) const {
    thrust::uniform_real_distribution<float> u01(0.f, 1.f);
    float prob = u01(rng);
    glm::vec4 sample = Phong_sampleOutWithPDF_Uniform(in, normal, rng);
    glm::vec3 out(sample);
    return MonteCarloPair(out, Phong_evalBSDF(in, normal, out, uv, prob) * (glm::max(0.f, glm::dot(out, normal)) / sample.w));
    ...
    //return MonteCarloPair(out, outColor * 2.f);
}
...
GLM_FUNC_QUALIFIER MonteCarloPair Material::Phong_sampleScatter_CosWeighted_Phong(const glm::vec3& in, const glm::vec3& normal, const glm::vec2& uv, thrust::default_random_engine& rng) const {
    glm::vec4 sample;
    float prob = 0.f;

    if (!hasReflective) {
        sample = Phong_sampleOutWithPDF_CosWeighted(in, normal, rng);
    }
    else {
        ...
        if (weightedProb <= diffuseWeight) {
            ...
            sample = Phong_sampleOutWithPDF_Phong(in, normal, rng);
        }
    }
    glm::vec3 out(sample);
    return MonteCarloPair(out, Phong_evalBSDF(in, normal, out, uv, prob) * (glm::max(0.f, glm::dot(out, normal)) / sample.w));
...
GLM_FUNC_QUALIFIER glm::vec4 Material::Phong_sampleOutWithPDF_Phong(const glm::vec3& in, const glm::vec3& normal, thrust::default_random_engine& rng) const {
    float pdf;
