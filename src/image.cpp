...
namespace Image {
    image::image(int x, int y) :
        xSize(x),
        ySize(y),
        pixels(new glm::vec3[static_cast<size_t>(x) * static_cast<size_t>(y)]) {
    }
    ...
    void image::savePNG(const std::string& baseFilename) {
        unsigned char* bytes = new unsigned char[3 * static_cast<size_t>(xSize) * static_cast<size_t>(ySize)];
